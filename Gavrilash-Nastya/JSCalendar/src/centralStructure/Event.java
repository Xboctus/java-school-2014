...
public class Event implements Cloneable, Serializable, Comparable<Event> {
	 ...
	 */
	public synchronized void correctInnerTime(int newOffsetinHours) {
		long sisOffset = new GregorianCalendar().getTimeZone().getRawOffset();
	...
	/**
	 * increase and return common for all Event class counter of ids.this is
	 * only one method, that has a deal with commonId field
	 * 
	 * @return
	 */
	private synchronized static int getCommonId() {
		commonId++;
		return commonId;
	}
