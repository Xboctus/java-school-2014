...
public class User implements Serializable {
	...
	public void removeEventFromUser(String text) throws IOException {
		...
		Scheduler.events.remove(events.get(text).getId());
		events.remove(text);
	 ...
	 * @throws IOException
	 */
	public void cloneEventFromUser(String eventText, User userTo)
			throws IOException {
		if (!events.containsKey(eventText)) {
			throw new IOException("This event doesn't exist");
		}
		if (Scheduler.users.get(userTo.getName()).events.containsKey(eventText)) {
			throw new IOException("This event already exist");
		}
		Event cloneEvent = (Event) events.get(eventText).clone();
		cloneEvent.correctInnerTime(Scheduler.users.get(userTo.getName())
				.getTimeZoneOffset());
		cloneEvent.setUser(userTo);
		Scheduler.users.get(userTo.getName()).addEventToUser(cloneEvent);
	}
}
