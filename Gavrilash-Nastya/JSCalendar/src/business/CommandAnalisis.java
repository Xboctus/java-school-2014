...
public class CommandAnalisis {
	...
	public static void createUserQuery(String[] tokens) throws IOException {
		...
		System.out.println("New user " + getName(tokens[1]) + " created");
	}
	...
	public static void modufyQuery(String[] tokens) throws IOException {
		...
		CommandHandler.modifyUser(getName(tokens[1]),
				getTimezoneOffset(tokens[2]), getActiveStatus(tokens[3]));
		System.out.println("User " + getName(tokens[1]) + " modified");
	}
	...
	public static void addEventQuery(String[] tokens) throws IOException {
		...
		System.out.println("Event " + getName(tokens[2]) + " added to user "
				+ getName(tokens[1]));
	}
	...
	public static void removeEventQuery(String[] tokens) throws IOException {
		...
		System.out.println("Event "
				+ getName(tokens[2] + " removed from user "
						+ getName(tokens[1])));
	}
	...
	public static void addRandomTimeEventQuery(String[] tokens)
		...
		System.out.println("RantTime event " + getName(tokens[2])
				+ " removed from user " + getName(tokens[1]));
	}
	...
	public static void cloneEventQuery(String[] tokens) throws IOException {
		...
		CommandHandler.cloneEvent(getName(tokens[1]), getName(tokens[2]),
				getName(tokens[3]));
		System.out.println("Event " + getName(tokens[2]) + " cloned from user "
				+ getName(tokens[1]) + " to user " + getName(tokens[3]));
	}
	...
	public static void showInfoQuery(String[] tokens) throws IOException {
		...
		// TODO with new methods in handler
	}
	...
	private static int getTimezoneOffset(String text) throws IOException {
	...
	private static String getName(String text) throws IOException {
		return text.trim();
	}

	private static boolean getActiveStatus(String text) throws IOException {
		text = text.replaceAll("\\s+", "");
		switch (text) {
		case "active":
			return true;
	...
	private static GregorianCalendar getGregorianCalendar(String text)
			throws IOException {
		text = text.replaceAll("\\s+", "");
		try {
